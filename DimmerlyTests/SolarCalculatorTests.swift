...
final class SolarCalculatorTests: XCTestCase {
    ...
    func testNewYorkSpringEquinox() throws {
        ...
        XCTAssertEqual(
            totalMinutes(sr.hour, sr.minute), totalMinutes(6, 58), accuracy: 3,
            "NYC equinox sunrise ~06:58, got \(sr.hour):\(String(format: "%02d", sr.minute))"
        )
        XCTAssertEqual(
            totalMinutes(ss.hour, ss.minute), totalMinutes(19, 10), accuracy: 3,
            "NYC equinox sunset ~19:10, got \(ss.hour):\(String(format: "%02d", ss.minute))"
        )
    }

    /// London — 2026-06-21 (summer solstice): sunrise ~04:43, sunset ~21:21 BST
    func testLondonSummerSolstice() throws {
        ...
        XCTAssertEqual(
            totalMinutes(sr.hour, sr.minute), totalMinutes(4, 43), accuracy: 3,
            "London solstice sunrise ~04:43, got \(sr.hour):\(String(format: "%02d", sr.minute))"
        )
        XCTAssertEqual(
            totalMinutes(ss.hour, ss.minute), totalMinutes(21, 21), accuracy: 3,
            "London solstice sunset ~21:21, got \(ss.hour):\(String(format: "%02d", ss.minute))"
        )
    }

    /// Tokyo — 2026-12-21 (winter solstice): sunrise ~06:47, sunset ~16:32 JST
    func testTokyoWinterSolstice() throws {
        let tz = try XCTUnwrap(TimeZone(identifier: "Asia/Tokyo"))
        ...
        XCTAssertEqual(
            totalMinutes(sr.hour, sr.minute), totalMinutes(6, 47), accuracy: 3,
            "Tokyo solstice sunrise ~06:47, got \(sr.hour):\(String(format: "%02d", sr.minute))"
        )
        XCTAssertEqual(
            totalMinutes(ss.hour, ss.minute), totalMinutes(16, 32), accuracy: 3,
            "Tokyo solstice sunset ~16:32, got \(ss.hour):\(String(format: "%02d", ss.minute))"
        )
    }
    ...
    func testEquatorDaylength() throws {
    ...
    func testSouthernHemisphereSeasons() throws {
        let tz = try XCTUnwrap(TimeZone(identifier: "Australia/Sydney"))
