...
//  DimmerlyTests
//
//  Unit tests for KeyboardShortcut functionality.
...
/// Tests for the KeyboardShortcut model
final class KeyboardShortcutTests: XCTestCase {
    ...
    func testDisplayString() {
        ...
        let customShortcut = KeyboardShortcut(key: "s", modifiers: ["control", "command"])
    ...
    func testDisplayStringModifierOrder() {
        ...
        let shortcut = KeyboardShortcut(key: "a", modifiers: ["command", "control", "option", "shift"])
    ...
    func testCodableConformance() throws {
        ...
        let originalShortcut = KeyboardShortcut(key: "r", modifiers: ["command", "shift"])
    ...
    func testEquatableConformance() {
        ...
        let shortcut1 = KeyboardShortcut(key: "d", modifiers: ["command", "option", "shift"])
        let shortcut2 = KeyboardShortcut(key: "d", modifiers: ["command", "option", "shift"])

        // Then: They should be equal
        XCTAssertEqual(shortcut1, shortcut2, "Identical shortcuts should be equal")

        // Given: Two different shortcuts
        let shortcut3 = KeyboardShortcut(key: "s", modifiers: ["command"])
    ...
    func testFromKeyCodeAndModifiers() {
        ...
        if let shortcut = KeyboardShortcut.from(keyCode: 2, modifierFlags: modifierFlags) {
        ...
        if let shortcut = KeyboardShortcut.from(keyCode: 2, modifierFlags: multiModifiers) {
            XCTAssertEqual(shortcut.key, "d", "Should create shortcut with 'd' key")
    ...
    func testUnsupportedKeyCode() {
        ...
        let shortcut = KeyboardShortcut.from(keyCode: 999, modifierFlags: modifierFlags)
    ...
    func testSupportedKeyCodes() {
        ...
        if let shortcut = KeyboardShortcut.from(keyCode: letterKeyCode, modifierFlags: [.command]) {
        ...
        if let shortcut = KeyboardShortcut.from(keyCode: numberKeyCode, modifierFlags: [.command]) {
